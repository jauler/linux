...
#define BCM2708_SPI_CS			0x00
#define BCM2708_SPI_FIFO		0x04
#define BCM2708_SPI_CLK			0x08
#define BCM2708_SPI_DLEN		0x0c
#define BCM2708_SPI_LTOH		0x10
#define BCM2708_SPI_DC			0x14

/* Bitfields in CS */
#define BCM2708_SPI_CS_LEN_LONG		0x02000000
#define BCM2708_SPI_CS_DMA_LEN		0x01000000
#define BCM2708_SPI_CS_CSPOL2		0x00800000
#define BCM2708_SPI_CS_CSPOL1		0x00400000
#define BCM2708_SPI_CS_CSPOL0		0x00200000
#define BCM2708_SPI_CS_RXF		0x00100000
#define BCM2708_SPI_CS_RXR		0x00080000
#define BCM2708_SPI_CS_TXD		0x00040000
#define BCM2708_SPI_CS_RXD		0x00020000
#define BCM2708_SPI_CS_DONE		0x00010000
#define BCM2708_SPI_CS_LEN		0x00002000
#define BCM2708_SPI_CS_REN		0x00001000
#define BCM2708_SPI_CS_ADCS		0x00000800
#define BCM2708_SPI_CS_INTR		0x00000400
#define BCM2708_SPI_CS_INTD		0x00000200
#define BCM2708_SPI_CS_DMAEN		0x00000100
#define BCM2708_SPI_CS_TA		0x00000080
#define BCM2708_SPI_CS_CSPOL		0x00000040
#define BCM2708_SPI_CS_CLEAR_RX		0x00000020
#define BCM2708_SPI_CS_CLEAR_TX		0x00000010
#define BCM2708_SPI_CS_CPOL		0x00000008
#define BCM2708_SPI_CS_CPHA		0x00000004
#define BCM2708_SPI_CS_CS_10		0x00000002
#define BCM2708_SPI_CS_CS_01		0x00000001

#define BCM2708_SPI_TIMEOUT_MS	30000
#define BCM2708_SPI_MODE_BITS	(SPI_CPOL | SPI_CPHA | SPI_CS_HIGH | SPI_NO_CS)
...
static inline void bcm2708_rd_fifo(struct bcm2708_spi *bs, int len)
{
	...
	while (len--) {
		byte = bcm2708_rd(bs, BCM2708_SPI_FIFO);
		if (bs->rx_buf)
			*bs->rx_buf++ = byte;
	}
}
...
static inline void bcm2708_wr_fifo(struct bcm2708_spi *bs, int len)
{
	...
	while (len--) {
		...
		bcm2708_wr(bs, BCM2708_SPI_FIFO, byte);
		bs->len--;
	}
}
...
static irqreturn_t bcm2708_spi_interrupt(int irq, void *dev_id)
{
	...
	u32 cs = bcm2708_rd(bs, BCM2708_SPI_CS);
	 ...
	 * bcm2708_spi_finish_transfer() drain the RX FIFO.
	 */
	if (bs->len && (cs & BCM2708_SPI_CS_RXR)) {
	 ...
	 */
	if (cs & BCM2708_SPI_CS_DONE) {
		if (bs->len) { /* First interrupt in a transfer */
			bcm2708_wr_fifo(bs, 16);
		} else { /* Transfer complete */
			/* Disable SPI interrupts */
			cs &= ~(BCM2708_SPI_CS_INTR | BCM2708_SPI_CS_INTD);
			bcm2708_wr(bs, BCM2708_SPI_CS, cs);

			/*
			 * Wake up bcm2708_spi_transfer_one(), which will call
			 * bcm2708_spi_finish_transfer(), to drain the RX FIFO.
			 */
			complete(&bs->done);
		}
		...
		struct spi_transfer *tfr)
{
	...
	u32 cs = BCM2708_SPI_CS_INTR | BCM2708_SPI_CS_INTD | BCM2708_SPI_CS_TA;
	...
	if (spi->mode & SPI_CPOL)
		cs |= BCM2708_SPI_CS_CPOL;
	if (spi->mode & SPI_CPHA)
		cs |= BCM2708_SPI_CS_CPHA;

	if (!(spi->mode & SPI_NO_CS)) {
		if (spi->mode & SPI_CS_HIGH) {
			cs |= BCM2708_SPI_CS_CSPOL;
			cs |= BCM2708_SPI_CS_CSPOL0 << spi->chip_select;
		}
	...
	bcm2708_wr(bs, BCM2708_SPI_CLK, cdiv);
	/*
		...
		struct spi_transfer *tfr, bool cs_change)
{
	...
	/* Drain RX FIFO */
	while (cs & BCM2708_SPI_CS_RXD) {
		bcm2708_rd_fifo(bs, 1);
		cs = bcm2708_rd(bs, BCM2708_SPI_CS);
	}
	...
	if (cs_change)
		/* Clear TA flag */
		bcm2708_wr(bs, BCM2708_SPI_CS, cs & ~BCM2708_SPI_CS_TA);
		...
		struct spi_message *mesg)
{
	...
	list_for_each_entry(tfr, &mesg->transfers, transfer_list) {
		...
		timeout = wait_for_completion_timeout(&bs->done,
				msecs_to_jiffies(BCM2708_SPI_TIMEOUT_MS));
		if (!timeout) {
			err = -ETIMEDOUT;
			goto out;
		}
...
static int bcm2708_spi_probe(struct platform_device *pdev)
{
	...
	master->mode_bits = BCM2708_SPI_MODE_BITS;
	...
	err = request_irq(bs->irq, bcm2708_spi_interrupt, 0,
...
MODULE_AUTHOR("Chris Boot <bootc@bootc.net>, Rytis Karpuška <rytis@elektromotus.lt>");
